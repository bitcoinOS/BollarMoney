import React, { useState, useEffect } from 'react';
import { useWallet } from '../contexts/WalletContext';
import { api } from '../services/api';

/**
 * 清算组件
 */
const Liquidation = () => {
  const { wallet } = useWallet();
  const [liquidatablePositions, setLiquidatablePositions] = useState([]);
  const [selectedPosition, setSelectedPosition] = useState(null);
  const [bollarAmount, setBollarAmount] = useState('');
  const [liquidationOffer, setLiquidationOffer] = useState(null);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState(null);
  const [success, setSuccess] = useState(null);
  const [step, setStep] = useState(1); // 1: 选择头寸, 2: 输入金额, 3: 确认交易, 4: 交易结果

  // 加载可清算头寸
  useEffect(() => {
    const loadLiquidatablePositions = async () => {
      if (!wallet.isConnected) return;

      setIsLoading(true);
      setError(null);

      try {
        const positions = await api.getLiquidatablePositions();
        setLiquidatablePositions(positions);
      } catch (err) {
        console.error('Failed to load liquidatable positions:', err);
        setError('加载可清算头寸失败，请稍后再试。');
      } finally {
        setIsLoading(false);
      }
    };

    loadLiquidatablePositions();

    // 设置定时刷新
    const intervalId = setInterval(loadLiquidatablePositions, 30000); // 每30秒刷新一次

    return () => clearInterval(intervalId);
  }, [wallet.isConnected]);

  // 处理头寸选择
  const handlePositionSelect = (position) => {
    setSelectedPosition(position);
    setBollarAmount(position.bollar_debt.toString());
    setStep(2);
  };

  // 处理 Bollar 金额输入变化
  const handleBollarAmountChange = async (e) => {
    const value = e.target.value;
    setBollarAmount(value);
    
    if (!value || !selectedPosition || isNaN(parseFloat(value))) {
      setLiquidationOffer(null);
      return;
    }
    
    // 验证金额
    const bollarValue = parseFloat(value);
    if (bollarValue <= 0 || bollarValue > selectedPosition.bollar_debt) {
      setLiquidationOffer(null);
      return;
    }
    
    try {
      // 调用预清算查询
      const offer = await api.preLiquidate(
        selectedPosition.position_id,
        Math.floor(bollarValue)
      );
      setLiquidationOffer(offer);
    } catch (err) {
      console.error('Failed to get liquidation offer:', err);
      setError('获取清算报价失败。');
      setLiquidationOffer(null);
    }
  };

  // 处理表单提交
  const handleSubmit = async (e) => {
    e.preventDefault();
    
    if (!wallet.isConnected) {
      setError('请先连接钱包。');
      return;
    }
    
    if (!selectedPosition || !bollarAmount || !liquidationOffer) {
      setError('请填写所有必填字段。');
      return;
    }
    
    // 验证金额
    const bollarValue = parseFloat(bollarAmount);
    if (bollarValue <= 0 || bollarValue > selectedPosition.bollar_debt) {
      setError(`Bollar 金额必须在 1 到 ${selectedPosition.bollar_debt} 之间。`);
      return;
    }
    
    // 进入确认步骤
    setStep(3);
  };

  // 执行清算
  const executeLiquidate = async () => {
    setIsLoading(true);
    setError(null);
    
    try {
      // 在实际实现中，这里需要构建 PSBT 交易
      // 这里简化处理，假设已经有了签名的 PSBT
      const signedPsbt = "dummy_signed_psbt";
      
      // 调用后端执行清算
      const result = await api.executeLiquidate(
        selectedPosition.position_id,
        signedPsbt
      );
      
      setSuccess(`清算成功！交易 ID: ${result}`);
      setStep(4);
    } catch (err) {
      console.error('Failed to execute liquidation:', err);
      setError(`清算失败: ${err.message}`);
    } finally {
      setIsLoading(false);
    }
  };

  // 重置表单
  const resetForm = () => {
    setSelectedPosition(null);
    setBollarAmount('');
    setLiquidationOffer(null);
    setError(null);
    setSuccess(null);
    setStep(1);
  };

  // 格式化 BTC 金额
  const formatBtc = (satoshis) => {
    return (satoshis / 100000000).toFixed(8);
  };

  if (!wallet.isConnected) {
    return (
      <div className="liquidation">
        <div className="not-connected">
          <h2>请先连接钱包</h2>
          <p>您需要连接比特币钱包才能进行清算操作。</p>
        </div>
      </div>
    );
  }

  if (isLoading && step === 1) {
    return (
      <div className="liquidation">
        <div className="loading">
          <p>加载中...</p>
        </div>
      </div>
    );
  }

  return (
    <div className="liquidation">
      <h2>清算</h2>
      
      {error && <div className="error-message">{error}</div>}
      {success && <div className="success-message">{success}</div>}
      
      {step === 1 && (
        <div className="position-selection">
          <h3>可清算头寸</h3>
          
          {liquidatablePositions.length === 0 ? (
            <p className="no-positions">当前没有可清算的头寸。</p>
          ) : (
            <div className="positions-list">
              {liquidatablePositions.map((position) => (
                <div key={position.position_id} className="position-card" onClick={() => handlePositionSelect(position)}>
                  <div className="position-header">
                    <span className="position-id">{position.position_id.substring(0, 8)}...</span>
                    <span className="position-owner">{position.owner.substring(0, 6)}...</span>
                  </div>
                  <div className="position-details">
                    <div className="detail-item">
                      <span className="detail-label">BTC 抵押:</span>
                      <span className="detail-value">{formatBtc(position.btc_collateral)} BTC</span>
                    </div>
                    <div className="detail-item">
                      <span className="detail-label">Bollar 债务:</span>
                      <span className="detail-value">{position.bollar_debt} BOLLAR</span>
                    </div>
                    <div className="detail-item">
                      <span className="detail-label">健康因子:</span>
                      <span className="detail-value" style={{ color: 'red' }}>
                        {(position.health_factor / 100).toFixed(2)}
                      </span>
                    </div>
                  </div>
                  <button className="select-button">清算</button>
                </div>
              ))}
            </div>
          )}
        </div>
      )}
      
      {step === 2 && selectedPosition && (
        <form onSubmit={handleSubmit} className="liquidation-form">
          <div className="selected-position">
            <h3>已选头寸</h3>
            <div className="position-details">
              <div className="detail-item">
                <span className="detail-label">头寸 ID:</span>
                <span className="detail-value">{selectedPosition.position_id.substring(0, 8)}...</span>
              </div>
              <div className="detail-item">
                <span className="detail-label">所有者:</span>
                <span className="detail-value">{selectedPosition.owner.substring(0, 6)}...</span>
              </div>
              <div className="detail-item">
                <span className="detail-label">BTC 抵押:</span>
                <span className="detail-value">{formatBtc(selectedPosition.btc_collateral)} BTC</span>
              </div>
              <div className="detail-item">
                <span className="detail-label">Bollar 债务:</span>
                <span className="detail-value">{selectedPosition.bollar_debt} BOLLAR</span>
              </div>
              <div className="detail-item">
                <span className="detail-label">健康因子:</span>
                <span className="detail-value" style={{ color: 'red' }}>
                  {(selectedPosition.health_factor / 100).toFixed(2)}
                </span>
              </div>
            </div>
          </div>
          
          <div className="form-group">
            <label htmlFor="bollarAmount">清算 Bollar 金额</label>
            <input
              type="number"
              id="bollarAmount"
              value={bollarAmount}
              onChange={handleBollarAmountChange}
              placeholder="输入 Bollar 金额"
              step="1"
              min="1"
              max={selectedPosition.bollar_debt}
              required
            />
            <div className="input-actions">
              <button
                type="button"
                className="max-button"
                onClick={() => handleBollarAmountChange({ target: { value: selectedPosition.bollar_debt.toString() } })}
              >
                最大
              </button>
            </div>
          </div>
          
          {liquidationOffer && (
            <div className="info-panel">
              <div className="info-item">
                <span className="info-label">清算奖励:</span>
                <span className="info-value">{formatBtc(liquidationOffer.liquidation_bonus)} BTC</span>
              </div>
              <div className="info-item">
                <span className="info-label">获得 BTC:</span>
                <span className="info-value">
                  {formatBtc(
                    (selectedPosition.btc_collateral * parseFloat(bollarAmount)) / 
                    selectedPosition.bollar_debt + liquidationOffer.liquidation_bonus
                  )} BTC
                </span>
              </div>
            </div>
          )}
          
          <button type="submit" className="submit-button" disabled={isLoading || !liquidationOffer}>
            {isLoading ? '处理中...' : '继续'}
          </button>
          <button type="button" onClick={resetForm} className="cancel-button">
            返回
          </button>
        </form>
      )}
      
      {step === 3 && selectedPosition && liquidationOffer && (
        <div className="confirmation">
          <h3>确认清算</h3>
          
          <div className="confirmation-details">
            <div className="detail-item">
              <span className="detail-label">清算 Bollar:</span>
              <span className="detail-value">{bollarAmount} BOLLAR</span>
            </div>
            <div className="detail-item">
              <span className="detail-label">获得 BTC:</span>
              <span className="detail-value">
                {formatBtc(
                  (selectedPosition.btc_collateral * parseFloat(bollarAmount)) / 
                  selectedPosition.bollar_debt + liquidationOffer.liquidation_bonus
                )} BTC
              </span>
            </div>
            <div className="detail-item">
              <span className="detail-label">清算奖励:</span>
              <span className="detail-value">{formatBtc(liquidationOffer.liquidation_bonus)} BTC</span>
            </div>
          </div>
          
          <div className="confirmation-actions">
            <button onClick={executeLiquidate} className="confirm-button" disabled={isLoading}>
              {isLoading ? '处理中...' : '确认清算'}
            </button>
            <button onClick={() => setStep(2)} className="back-button" disabled={isLoading}>
              返回
            </button>
          </div>
        </div>
      )}
      
      {step === 4 && (
        <div className="result">
          <h3>清算结果</h3>
          
          {success && (
            <div className="success-details">
              <p>{success}</p>
              <button onClick={resetForm} className="new-transaction-button">
                新建清算
              </button>
              <a href="/dashboard" className="dashboard-link">
                查看仪表板
              </a>
            </div>
          )}
        </div>
      )}
    </div>
  );
};

export default Liquidation;